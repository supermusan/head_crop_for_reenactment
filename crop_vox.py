...
def split_in_utterance(person_id, video_id, args):
    # 官方标注中给出了人脸片段的起始和结束帧，据此从视频中切出人脸
    video_path = os.path.join(args.video_folder, video_id + ".mp4")
...
def run(params):
    ...
    for video_id in os.listdir(video_folder):
        ...
        try:
            if args.download:
                print("args.download is set as 'True'.")
                return
...
if __name__ == "__main__":
    ...
    import copy
    ids_temp = sorted(list(copy.deepcopy(ids)))
    # print(len(ids))
    # print((args.data_range[0], args.data_range[1]))
