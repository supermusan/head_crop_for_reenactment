...
def scheduler_count(data_list, fn, args):
    device_ids = args.device_ids.split(",")
    pool = Pool(processes=args.workers)
    args_list = cycle([args])
    summary = 0
    for num in tqdm(pool.imap_unordered(fn, zip(data_list, cycle(device_ids), args_list))):
        summary += num
    print(summary)


def save(path, frames, format, fps=10):
    if format == '.mp4':
